          ...
          <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
            <header>
              ...
              <p className="mt-4 max-w-md text-gray-500">
                Temukan pilihan produk terpopuler kami yang paling banyak
        ...
        <section>
          <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
            <header>
              ...
              <p className="mt-4 max-w-md text-gray-500">
                Jelajahi berbagai kategori baju yang kami sediakan. Temukan
                koleksi yang sesuai dengan aktivitas dan selera kamu—semua
                dirancang untuk memberikan kenyamanan dan tampilan terbaik di
                setiap kesempatan.
              </p>
            </header>
...
const Testomonials = () => {
  const testimonials = useLiveQuery(() =>
    dexie.testimonySettings
      .filter((x) => x.name !== "" && x.testimony !== "")
      .sortBy("id"),
  );
  return (
    <>{testimonials?.map((x) => <TestimonialCard data={x} key={x.id} />)}</>
  );
};
...
const CarouselImages = () => {
  const carousel = useLiveQuery(() =>
    dexie.carouselSettings.filter((x) => x.imageUrl !== "").sortBy("id"),
  );
  return <Carousel images={carousel?.map((x) => x.imageUrl) ?? []} />;
};
...
const CategoryCard: React.FC<{ data: DB<"CategorySetting"> }> = ({ data }) => {
  return (
    <li>
      <Link href={data.url} className="group block overflow-hidden">
...
const TestimonialCard: React.FC<{ data: DB<"TestimonySetting"> }> = ({
  data,
}) => {
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>{data.job}</CardDescription>
      ...
      <CardContent>
        <ScrollArea className="max-h-[200px]">
          <p className="mr-4">{data.testimony}</p>
